"""
AI SDR - Main Entry Point (Simplified Version)
Uses real Apify actor: harvestapi/linkedin-profile-search
"""
import os
import sys
import logging
...
def run_pipeline(config: dict, max_companies: int = None, max_profiles: int = None):
    ...
    logger.info("=" * 60)
    logger.info("Starting AI SDR Pipeline (Simplified)")
    logger.info("=" * 60)
    ...
    total_profiles = max_companies * max_profiles_per_company

    logger.info(f"Limits: {max_companies} companies × {max_profiles_per_company} profiles = {total_profiles} total profiles")
    ...
    try:
        ...
        # Step 2: Scrape Profiles Directly
        logger.info("\n[Step 2/5] Scraping LinkedIn Profiles via Apify")
        logger.info("Using actor: harvestapi/linkedin-profile-search")
        ...
        logger.info(f"✓ Scraped {len(all_profiles)} total profiles")
        ...
        logger.info("=" * 60)
        logger.info(f"Target: {max_companies} companies × {max_profiles_per_company} profiles")
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='AI SDR - Automated Lead Generation (Simplified)')
    ...
    if args.schedule:
        ...
        # One-time run
        logger.info("Running in one-time mode (Simplified)")
        run_pipeline(config, args.max_companies, args.max_profiles)
